...
def distance_func2(nside,RA_RM,Dec_RM,gal_RA,gal_Dec,kpc2deg_list, radii_list, z_bin_list):
    ...
    for i in np.arange(0,len(Dec_RM)):
        ...
        RM_vec = hp.ang2vec(Dec_RM_rad,RA_RM_rad)


        pix_matrix = hp.query_disc(nside, RM_vec, r_rad, inclusive=True)
...
def mask(grid, RM_RA, RM_Dec, RM, radii_list):
	...
	return np.array(new_RM_RA), np.array(new_RM_Dec), np.array(new_RM)


def mask2(grid_vals, RA_RM, Dec_RM, RM, radii_list):
	...
	for i in np.arange(0,len(Dec_RM)):
		...
		#should this be Dec_RM - 90?
		#should I change it to np.absolute(90 - Dec_RM)?
		'''
		Dec_RM_rad=(np.pi/180.)*(90.-Dec_RM[i])
		RA_RM_rad=(np.pi/180.)*(RA_RM[i])
		'''

		Dec_RM_rad, RA_RM_rad = DeclRaToThetaPhi(Dec_RM[i],RA_RM[i])

		Dec_min, RA_min = DeclRaToThetaPhi(Dec_min,RA_min) #(np.pi/180.)*(90.-Dec_min)
		#RA_min=(np.pi/180.)*(RA_min)
		Dec_max, RA_max = DeclRaToThetaPhi(Dec_max,RA_max)
		'''
	...
	return np.array(RA_RM_keep), np.array(Dec_RM_keep), np.array(RM_return) #new_RM_RA, new_RM_Dec, new_RM, RM_delete, RM_keep



def mask_gal_plane():
	#put ra/dec into SkyCoords
	#change into b and l
...
def CCF_theta2(cln,l_start=0,end_theta=2.,theta_samples=50):

    ells=np.arange(cln.size)

    lfact=(2*ells+1)/(4*np.pi)

    cl_theta=np.zeros(theta_samples, dtype =float)

    ctheta=np.linspace(0.,end_theta,num=theta_samples)

    pls=np.zeros((theta_samples,cln.size))

    cthetar=np.radians(ctheta)

    

    for i in range(cln.size):

        pls[:,i]=eval_legendre(float(i),np.cos(cthetar))

    

    aa=pls*lfact[np.newaxis,:]

    cl_theta=(aa*cln[np.newaxis,:]).sum(axis=1)
...
def predict_numbdense(total_gal_in_z,z_mean,radii_list,sky_frac):
    ...
    zweight=((1+np.array(z_mean))**(-2.))*(1./np.array(total_gal_in_z))
    zweight=zweight/zweight.sum()*4.
    #zweight = ((1+np.array(z_mean))**(-2.))
    avegalzwt=avegalz*zweight[:,np.newaxis]#zweight[:,np.newaxis]
    ...
    #return np.array([avegalz_sqkpc,avegalzwt_sqkpc]),np.array([avegalz,avegalzwt]),np.array([avegal_sqkpc,avegal_sqkpc_wt]),np.array([avegal,avegal_sqkpc_wt])
    return avegal_wt

def gal_sources_hist(RA_RM, Dec_RM, RM_vals, dist_matrix_kpc, radii_list):

	radial_bins = radii_list 

	#calculating the average galaxy counts

	total_gal_num= np.zeros((len(RA_RM),len(radial_bins)-1))
	...
	for k in np.arange(0,len(RA_RM)): #can change this to only look at the first few
		...
		gal_counts_single, bin_edges = np.histogram(RM_dist_grid, bins=radial_bins)

		total_gal_num[k] = gal_counts_single

	avg_gal_num = np.average(total_gal_num, axis = 0) 

	avg_gal_std = np.std(total_gal_num, axis = 0)

	return avg_gal_num, bin_edges, avg_gal_std, total_gal_num


def cross_corr_predict(RA_RM, Dec_RM, RM_vals, dist_matrix_kpc, radii_list, z_mean, total_gal_in_z, dist_z_split,avegal_wt):
	#input the galaxy catalogue number density (the 2048 grid one), for each redshift
	#input the RM catalogue 
	...
	##############
	zwt = ((1+np.array(z_mean))**(-2.))*((total_gal_in_z)**(-1.))
	zwt=zwt/zwt.sum()*4.
	#print "z-weighting term: ", zwt
	...
	for k in np.arange(0,len(RA_RM)): #can change this to only look at the first few
		...
		for i in np.arange(0, len(z_mean)):
			...
			#weight it myself (it is a bit sketchy inside the histogram)
			gal_counts_radial_z = gal_counts_radial_z*zwt[i]
	...
	delt_rho_r = (rho_r - avegal_wt)/areabins1# 2dlist - 1dlist
	...
	cross_corr_func = rho_RM#/avegal_wt #numerator/denom, appending to a list for various r-values

	return cross_corr_func


def classical_cross_corr(RA_RM, Dec_RM, RM_vals, dist_matrix_kpc, radii_list, z_mean, total_gal_in_z, dist_z_split):
	#input the galaxy catalogue number density (the 2048 grid one), for each redshift
	#input the RM catalogue 
...
def cross_corr(RA_RM, Dec_RM, RM_vals, dist_matrix_kpc, radii_list, z_mean, total_gal_in_z, dist_z_split):
	...
	for k in np.arange(0,len(RA_RM)): #can change this to only look at the first few
		...
		gal_counts_radial = np.zeros(len(radial_bins)-1)
		#will not use redshift bins for now!

		gal_counts_single, bin_edges = np.histogram(RM_dist_grid, bins=radial_bins)

		gal_counts_avg[k] += gal_counts_single
	...
	std_rho = np.std(delt_rho_r, axis = 0)
	std_RM =  np.std(delt_RM)


	cross_corr_func = rho_RM/(std_rho*std_RM) #numerator/denom, appending to a list for various r-values
...
def cross_corr_plot_scramble(radii_list, cross_corr_func, cross_corr_real,n, cross_corr_err):
	#plotting the cross correlation function
	radii_list = [(a + b) /2. for a, b in zip(radii_list[::], radii_list[1::])]
	plt.figure()
	plt.plot(radii_list, radii_list, 'r-', linewidth = 2, markersize = 30, label = 'Scrambeled RM for '+str(n)+' runs')
	plt.plot(radii_list, cross_corr_func, 'b+', markersize = 30)
	plt.errorbar(radii_list, radii_list, yerr = cross_corr_err)
	...
	plt.xscale('log')
	#put an x lim on this!!
	plt.figtext(0,0,"Subtracting galactic foreground with Niels' foreground, scrambling RM sources - predicted mean")
...
def DeclRaToThetaPhi(decl,RA):
    return np.radians(-decl+90.),np.radians(360.-RA)
'''
